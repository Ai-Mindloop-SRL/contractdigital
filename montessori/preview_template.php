...
// Fetch all fields for this template using template_field_mapping
$stmt = $conn->prepare("
    SELECT 
        fd.field_name,
        fd.field_label,
...
foreach ($fields as $field) {
    ...
    // Use placeholder as hint if exists (but don't use as actual data)
    // We'll generate appropriate sample data based on field name patterns
...
// Replace placeholders with highlighted sample data (without adding data-field, template already has it)
foreach ($sample_data as $field_name => $field_value) {
    $preview_content = str_replace(
        '[' . $field_name . ']',
        '<span class="filled-field" title="' . htmlspecialchars($field_name . ': ' . $field_value) . '">' 
        . htmlspecialchars($field_value) . '</span>',
        $preview_content
    );
}

// Highlight any remaining unfilled placeholders (without adding data-field, template already has it)
$preview_content = preg_replace_callback(
    '/\[([A-Z_0-9]+)\]/',
    function($matches) {
        return '<span class="unfilled-field" title="Câmp lipsă: ' . htmlspecialchars($matches[1]) . '">' . htmlspecialchars($matches[0]) . '</span>';
    },
    $preview_content
);

// Count fields
// 1. Unique fields (distinct field names from database)
$unique_fields = count($fields);

// 2. Total placeholders (ALL occurrences in original template HTML, including duplicates)
// Count ALL data-field attributes from ORIGINAL template content
preg_match_all('/data-field="([^"]+)"/', $template['template_content'], $template_fields_matches);
$total_placeholders = count($template_fields_matches[1]); // Total occurrences, not unique

// 3. Count unfilled placeholders in preview
$unfilled_count = preg_match_all('/class="unfilled-field"/', $preview_content);

// For display
$filled_count = count($sample_data);
