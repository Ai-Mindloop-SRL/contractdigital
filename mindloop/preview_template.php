...
foreach ($sample_data as $field_name => $field_value) {
    ...
    // Suportă UPPERCASE și lowercase placeholders
    $placeholder_upper = '[' . strtoupper($clean_field_name) . ']';
    $placeholder_lower = '[' . strtolower($clean_field_name) . ']';
    
    // Replace both uppercase și lowercase
    $filled_html = '<span class="filled-field" data-field="' . htmlspecialchars($clean_field_name) . '" title="' . htmlspecialchars($clean_field_name . ': ' . $field_value) . '">' 
        . htmlspecialchars($field_value) . '</span>';
    
    $preview_content = str_replace($placeholder_upper, $filled_html, $preview_content);
    $preview_content = str_replace($placeholder_lower, $filled_html, $preview_content);
}

// Highlight any remaining unfilled placeholders (UPPERCASE și lowercase)
...
// Count fields
// 1. Unique fields (distinct field names from preview content) - UPPERCASE și lowercase
...
// 2. Total placeholders (total occurrences in template) - UPPERCASE și lowercase
