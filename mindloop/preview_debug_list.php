...
<!-- DEBUG PANEL -->
<div style='position:fixed;top:60px;right:10px;background:#0066cc;color:white;padding:15px;z-index:9999;max-width:350px;font-size:11px;border-radius:8px;max-height:80vh;overflow-y:auto;'>
    <strong>🔍 PLACEHOLDERS GĂSITE:</strong><br><br>
    
    <strong>Unique în template (<?php echo $unique_fields; ?>):</strong><br>
    <div style='background:rgba(255,255,255,0.2);padding:5px;margin:5px 0;border-radius:4px;'>
    <?php echo implode(', ', array_unique($template_placeholders[1])); ?>
    </div>
    
    <strong>Total în preview (<?php echo $total_placeholders; ?>):</strong><br>
    <div style='background:rgba(255,255,255,0.2);padding:5px;margin:5px 0;border-radius:4px;'>
    <?php echo implode(', ', array_unique($all_fields_matches[1])); ?>
    </div>
    
    <strong>Unfilled (<?php echo $unfilled_count; ?>):</strong><br>
    <div style='background:rgba(255,255,255,0.2);padding:5px;margin:5px 0;border-radius:4px;'>
    <?php 
    if (!empty($unfilled_matches[1])) {
        echo implode(', ', array_unique($unfilled_matches[1]));
    } else {
        echo 'None';
    }
    ?>
